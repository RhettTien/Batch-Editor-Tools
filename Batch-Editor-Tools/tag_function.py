...
def add_tag(select_tag_value, add_tag_value, field_name_value, file_path_value):
    ...
    if(field_name_value == "" or add_tag_value == ""):
        QMessageBox.information(self, "Warning", "The Field Name or the Tag to be added cannot be null!",
                            QMessageBox.Yes)
        return
        
    # wrong file path
    if(len(file_path_value) < 2):
        QMessageBox.information(self, "Warning", "Must select a word list file to execute!",
                            QMessageBox.Yes)
        return
    # wrong file type
    file_type = file_path_value[file_path_value.rfind(".")+1:]
    if(file_type != "xls" and file_type != "xlsx" and file_type != "txt"):
        QMessageBox.information(self, "Warning", "Please select the correct file format!(.txt .xls .xlsx)",
                            QMessageBox.Yes)
        return
